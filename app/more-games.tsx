...
const rainConsonantRoman=["k","kh","g","gh","ch","chh","j","jh","tt","tth","dd","ddh","t","th","d","dh","n","p","ph","b","bh","m","y","r","l","v","sh","shh","s","h"];
const rainLettersRoman:Record<RainLanguage,Record<RainGroup,string[]>>={
  Telugu:{Vowels:["a","aa","i","ee","u","oo","ru","e","ae","ai","o","oa","au","am","aha"],Consonants:rainConsonantRoman},
  Hindi:{Vowels:["a","aa","i","ee","u","oo","ri","e","ai","o","au","am","aha"],Consonants:rainConsonantRoman},
};
...
function LetterRain(){
  ...
  const source=rainLetters[language][group];const targetLetterIndex=(round*5+2)%source.length;const target=source[targetLetterIndex];const targetRoman=rainLettersRoman[language][group][targetLetterIndex];const targetIndex=(round*3+1)%6;
  ...
  return <article className="letter-rain-game"><div className="letter-rain-head"><div><small>GAME 9 · SILENT LETTER RECOGNITION</small><h3>Letter Rain</h3><p>Find and tap the requested letter before it reaches the bottom.</p></div><b>⭐ {score} points</b></div><div className="rain-controls"><div>{(["Telugu","Hindi"] as const).map(item=><button key={item} className={language===item?"active":""} onClick={()=>restart(item,group)}>{item==="Telugu"?"తెలుగు":"हिन्दी"}</button>)}</div><div>{(["Vowels","Consonants"] as const).map(item=><button key={item} className={group===item?"active":""} onClick={()=>restart(language,item)}>{item}</button>)}</div></div><div className="rain-target">Tap <strong>{target}</strong><VoiceButton compact src={`/assets/audio/letters/${language.toLowerCase()}-${group.toLowerCase()}-${targetRoman}.wav`} playLabel={`Hear ${target}`} /></div><div className="rain-sky" key={`${language}-${group}-${round}`} aria-label={`Tap the falling letter ${target}`}>{drops.map((letter,index)=><button key={`${round}-${index}-${letter}`} style={{"--rain-left":`${8+index*16}%`,"--rain-delay":`${(index%3)*.35}s`,"--rain-speed":`${5.8+(index%2)*.7}s`} as React.CSSProperties} onClick={()=>choose(letter)} onAnimationEnd={index===targetIndex?missed:undefined} aria-label={`Falling letter ${letter}`}>{letter}</button>)}</div><div className="rain-feedback" aria-live="polite">{feedback}</div><button className="rain-next" onClick={()=>{setRound(value=>value+1);setFeedback("Tap the matching falling letter.");}}>New letter →</button></article>;
}
...
function EcoSortGame(){
  ...
  return <article className="mini-game eco-game">
    ...
    {phase==="play"&&<>
      ...
      <div className={`eco-leaf${feedback?feedback.ok?" flash-correct":" flash-wrong":""}`}><div className="eco-leaf-inner"><span className="eco-item-emoji">{shuffled[index].emoji}</span><span className="eco-item-name">{ecoLabel(shuffled[index],language)}</span><VoiceButton compact src={`/assets/audio/eco/${language.toLowerCase()}-${shuffled[index].id}.wav`} playLabel={`Hear ${ecoLabel(shuffled[index],language)}`} /></div></div>
...
function FindItForMithuGame(){
  return <article className="mini-game find-it-game"><div className="mini-game-head"><div><small>GAME 6 · FIND IT</small><h3>Find It for <em>Mithu</em></h3><p>Mithu asks where an object is in Telugu or Hindi — tap the matching picture card for a burst of sparkles.</p></div></div><iframe src="/find-it-for-mithu.html" title="Find It for Mithu" loading="lazy" style={{width:"100%",height:"620px",border:0,borderRadius:"24px",background:"#BFEFFF"}}/></article>;
}

type GameId="bubble-pop"|"memory"|"find-it-for-mithu"|"vocabulary"|"actions"|"letter-rain"|"first-letter"|"coloring"|"eco-sort";
type GameAccent="violet"|"coral"|"mint"|"gold";
type GameCatalogEntry={id:GameId;title:string;description:string;icon:string;accent:GameAccent;category:string;Component:()=>React.JSX.Element};

const gameCatalog:GameCatalogEntry[]=[
  {id:"bubble-pop",title:"Parrot Letter Balloon Pop",description:"Pop the balloon that matches the letter you hear or see.",icon:"🎈",accent:"mint",category:"Letters & Sounds",Component:BubblePopGame},
  {id:"memory",title:"Happy Match Adventure",description:"Tap two cards and find every matching pair before time runs out.",icon:"🃏",accent:"violet",category:"Memory & Sorting",Component:MemoryGame},
  {id:"find-it-for-mithu",title:"Find It for Mithu",description:"Mithu asks where an object is in Telugu or Hindi — tap the matching picture.",icon:"🔍",accent:"gold",category:"Words & Vocabulary",Component:FindItForMithuGame},
  {id:"vocabulary",title:"Drag, Match & Learn",description:"Drag each picture card onto the Telugu word it matches.",icon:"🥕",accent:"coral",category:"Words & Vocabulary",Component:VocabularyShuffle},
  {id:"actions",title:"Quick, Tap the Action!",description:"Read the word before the ring runs out, then tap the matching picture.",icon:"⏱️",accent:"mint",category:"Words & Vocabulary",Component:ActionFlashcards},
  {id:"letter-rain",title:"Letter Rain",description:"Find and tap the requested letter before it reaches the bottom.",icon:"🌧️",accent:"violet",category:"Letters & Sounds",Component:LetterRain},
  {id:"first-letter",title:"Drag the First Letter!",description:"Look at the picture, then drag the letter its name starts with.",icon:"🔡",accent:"coral",category:"Letters & Sounds",Component:FirstLetterMatch},
  {id:"coloring",title:"Vani's Magic Art Studio",description:"Pick a picture, choose a color, then tap inside the lines to fill it in.",icon:"🎨",accent:"gold",category:"Creativity & Our Earth",Component:ColoringStudio},
  {id:"eco-sort",title:"Paryavaran Hero",description:"Sort each item into the right bin and help the tree grow.",icon:"♻️",accent:"mint",category:"Creativity & Our Earth",Component:EcoSortGame},
];

export default function MoreGames(){
  const [activeGame,setActiveGame]=useState<GameId|null>(null);
  const active=gameCatalog.find(game=>game.id===activeGame);

  useEffect(()=>{document.querySelector(".learn-experience")?.scrollTo({top:0});},[activeGame]);

  if(active){
    const GameComponent=active.Component;
    return <section className="more-games" id="more-games">
      <div className="game-active-header"><button className="game-back" onClick={()=>setActiveGame(null)}>← All games</button><div className={`game-active-title accent-${active.accent}`}><span>{active.icon}</span><b>{active.title}</b></div></div>
      <GameComponent/>
    </section>;
  }

  return <section className="more-games" id="more-games"><div className="section-title"><p className="kicker"><b>✦</b> More playful practice</p><h2>Remember it.<br/><em>Sort it correctly.</em></h2><p>Short games for visual memory, word recognition, language practice, and careful thinking. Tap any game to start playing.</p></div>
  <div className="game-picker-grid">{gameCatalog.map(game=><button key={game.id} className={`game-picker-card accent-${game.accent}`} onClick={()=>setActiveGame(game.id)}><span className="game-picker-icon">{game.icon}</span><small className="game-picker-category">{game.category}</small><h3>{game.title}</h3><p>{game.description}</p><b>Play →</b></button>)}</div>
</section>;
}
